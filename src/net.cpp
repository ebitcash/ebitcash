...
void CNode::copyStats(CNodeStats &stats)
{
    ...
    // Raw ping time is in microseconds, but show it to user as whole seconds (eBitcash users should be well used to small numbers with many decimal places by now :)
...
void ThreadMapPort()
{
    ...
    if (r == 1)
    {
        ...
        std::string strDesc = "eBitcash " + FormatFullVersion();
...
void CConnman::ThreadDNSAddressSeed()
{
    ...
    LogPrintf("seed size:....%d\n", vSeeds.size());
    for (const CDNSSeedData &seed : vSeeds) {
        if (interruptNet) {
            //return;
        	LogPrintf("interruptNet\n");
        }
        //LogPrintf("seed:...%s\n", seed.host.c_str());
        //seed.host
        if (HaveNameProxy()) {
