...
bool CWallet::LoadCScript(const CScript& redeemScript)
{
    ...
    {
        std::string strAddr = CeBitcashAddress(CScriptID(redeemScript)).ToString();
...
std::map<CTxDestination, std::vector<COutput>> CWallet::ListCoins() const
{
    ...
    // https://github.com/ebitcash/ebitcash/pull/10340, but that change has been
    // postponed until after https://github.com/ebitcash/ebitcash/pull/10244 to
        ...
        LOCK2(cs_main, cs_wallet);
        {
            ...
            while (true)
            {
                ...
                if (nChange > 0)
                {
                    ...
                    // change transaction isn't always pay-to-ebitcash-address
                ...
                //std::string str = std::to_string( nFeeNeeded ) + "\n" + std::to_string( ::minRelayTxFee.GetFee(nBytes) );
                //strFailReason = _( str.c_str() );
                //return false;
...
bool CWallet::SetAddressBook(const CTxDestination& address, const std::string& strName, const std::string& strPurpose)
{
    ...
    if (!strPurpose.empty() && !CWalletDB(*dbw).WritePurpose(CeBitcashAddress(address).ToString(), strPurpose))
        return false;
    return CWalletDB(*dbw).WriteName(CeBitcashAddress(address).ToString(), strName);
}
...
{
    {
        ...
        std::string strAddress = CeBitcashAddress(address).ToString();
    ...
    CWalletDB(*dbw).ErasePurpose(CeBitcashAddress(address).ToString());
    return CWalletDB(*dbw).EraseName(CeBitcashAddress(address).ToString());
}
 ...
 * https://ebitcashtalk.org/?topic=54527, or
 * https://github.com/ebitcash/ebitcash/pull/1393.
 */
...
bool CWallet::AddDestData(const CTxDestination &dest, const std::string &key, const std::string &value)
{
    ...
    return CWalletDB(*dbw).WriteDestData(CeBitcashAddress(dest).ToString(), key, value);
}
...
bool CWallet::EraseDestData(const CTxDestination &dest, const std::string &key)
{
    if (!mapAddressBook[dest].destdata.erase(key))
        return false;
    return CWalletDB(*dbw).EraseDestData(CeBitcashAddress(dest).ToString(), key);
}
