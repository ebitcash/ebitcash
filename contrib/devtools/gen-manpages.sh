...
EBTCVER=($($EBITCASHCLI --version | head -n1 | awk -F'[ -]' '{ print $6, $7 }'))
...
for cmd in $EBITCASHD $EBITCASHCLI $EBITCASHTX $EBITCASHQT; do
  cmdname="${cmd##*/}"
  help2man -N --version-string=${EBTCVER[0]} --include=footer.h2m -o ${MANDIR}/${cmdname}.1 ${cmd}
  sed -i "s/\\\-${EBTCVER[1]}//g" ${MANDIR}/${cmdname}.1
done
