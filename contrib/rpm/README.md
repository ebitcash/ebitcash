...
The RPM spec file provided here is for eBitcash-Core 0.12.0 and builds on CentOS
...
reference the release tarball available from https://ebitcash.org/bin/ and
`Source1` should reference the BerkeleyDB source.

Sources 10-99 are for source files that are maintained in the
[eBitcash git repository](https://github.com/ebitcash/ebitcash) but are not part of
...
Currently only `Source10` is used, for the example `ebitcash.conf` file.
...
in the ebitcash git repository. At present only an SVG version of the eBitcash
icon is used.

## Patches

In general, patches should be avoided. When a packager feels a patch is
necessary, the packager should bring the problem to the attention of the ebitcash
...
needed when 0.12.1 is released, a proper fix is already in the eBitcash git
master branch.
...
idea to build eBitcash Core against a static version of that library compiled
...
The `ebitcash.svg` file is from the source listed as `Source100`. It is used as
...
matters for the `ebitcash-server` package. At this point, most RPM based
...
The `ebitcash-server` package should have SELinux support. How to properly do
...
eBitcash-Core against OpenSSL or use a patched version of Boost in the build
system.
...
[Ringing Liberty](https://www.ringingliberty.com/ebitcash/). He has been
packaging eBitcash for Fedora at least since 2012.
...
4. I renamed the `ebitcash` package that contains the Qt GUI to `ebitcash-core` as
that appears to be how the general population refers to it, in contrast to
`ebitcash-xt` or `ebitcash-classic`. I wanted to make sure the general population
